...
router.get('/', (req, res) => {
  ...
  if (sort) {
  const sortOptions = {
    createdAt: (a, b) => new Date(a.startDate) - new Date(b.startDate),
    dueDate: (a, b) => new Date(a.endDate) - new Date(b.endDate),
...
router.put('/:id', (req, res) => {
  ...
  todo.history.push(originalTodo);
...
router.get('/filter', (req, res) => {
  ...
  let filteredTodos = [...todos]; // 創建一個複本，以免修改原始資料
...
function filterByTime(todos, startDate, endDate) {
  const filteredTodos = todos.filter((todo) => {
    ...
    return todoStartDate >= startDate && todoEndDate <= endDate;
  });
